...
namespace BLL.Services.Implementations
{
    public class UserService : IUserService
    {
        ...
        public async Task<IEnumerable<UserModel>> GetAllUsersAsync()
        {
            ...
            {
                var bllUser = DALUserToBLLUser(user);
                if (bllUser != null)
                {
                    bllUsers.Add(bllUser);
                }
            }
        ...
        private async Task<Result<UserModel>> IsValidRegistrationData(UserModel user)
        {
            ...
            message += passwordValidity ? string.Empty : "Invalid password.\t";

            bool emailValidity = user.Email != null && new Regex("^\\S+@\\S+\\.\\S+$").IsMatch(user.Email);
            message += emailValidity ? string.Empty : "Invalid email.\t";

            bool requiredFieldsPresent = user.NickName != null && user.FirstName != null && user.LastName != null;
            message += requiredFieldsPresent ? string.Empty : "Required fields are missing.";
